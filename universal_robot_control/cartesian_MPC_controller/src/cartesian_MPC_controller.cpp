...
#include "cartesian_MPC_controller/kinematics_base.h"
#include "cartesian_MPC_controller/cartesian_MPC_controller.h"
#include "kdl_conversions/kdl_msg.h"

namespace cartesian_mpc_controller
{
/** \brief Initialize the kinematic chain for kinematics-based computation.
 *
 */
bool Cartesian_MPC_Controller::init(
  ...
  sub_command_ = n.subscribe("command_cart_vel", 5,
    &Cartesian_MPC_Controller::command_cart_vel, this,
  ...
  _dt = static_cast<double>(1.0/publish_rate_);
  _mpc_params["DT"] = _dt;
  _mpc_steps = 20.0;
  _mpc_params["STEPS"]    = _mpc_steps;
  _ref_q = 0.0;
  _mpc_params["REF_Q"]  = _ref_q;
  _weight_q = 100.0;
  _mpc_params["WEIGHT_Q"]    = _weight_q;
  _weight_tau = 100.0;
  _mpc_params["WEIGHT_TAU"]   = _weight_tau;
 ...
 */
void Cartesian_MPC_Controller::starting(const ros::Time& time){
 ...
 */
void Cartesian_MPC_Controller::update(const ros::Time& time, const ros::Duration& period) {
  // Get joint positions
  for(std::size_t i=0; i < this->joint_handles_.size(); i++)
  {
  ...
  VectorXd state(2);
  state   << this->joint_state_.q(0), this->joint_state_.qdot(0);
  VectorXd coeffs(2);
  coeffs  << 1.0, 1.0;
 ...
 */
void Cartesian_MPC_Controller::command_cart_vel(const geometry_msgs::TwistConstPtr &msg) {
 ...
 */
void Cartesian_MPC_Controller::writeVelocityCommands(
...
PLUGINLIB_EXPORT_CLASS(cartesian_mpc_controller::Cartesian_MPC_Controller,
