...
bool Cartesian_Position_Controller::init(
  ...
  End_Pose_Cmd_.p.Zero();
  End_Pose_Cmd_.M.Identity();
...
void Cartesian_Position_Controller::starting(const ros::Time& time){
  ...
  End_Vel_Cmd_ = KDL::Twist::Zero();
  fk_pos_solver_->JntToCart(this->joint_state_.q, End_Pose_Cmd_);
...
void Cartesian_Position_Controller::update(const ros::Time& time, const ros::Duration& period) {
  ...
  ik_pos_solver_->CartToJnt(this->joint_state_.q, End_Pose_Cmd_, Jnt_Pos_Cmd_);
 ...
 */
void Cartesian_Position_Controller::command_cart_pos(const geometry_msgs::Pose &msg) {
    tf::PoseMsgToKDL(msg, End_Pose_Cmd_);
}
...
void Cartesian_Position_Controller::writePositionCommands(
    ...
    for(std::size_t i=0; i < this->joint_handles_.size(); i++) {
      ...
      // this->joint_handles_[i].setCommand(0.0);
    }
}
